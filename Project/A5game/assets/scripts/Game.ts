...
export class game extends Component {
    ...
    // ly
    //三消食材之后，第一个成功面板
    @property(Node) match3SuccessFirstPanel: Node; // 面板的弹窗
    @property(Node) match3SuccessFirstPanelButton1: Node; // 弹窗的第一个按钮
    //三消食材之后，第二个成功面板
    @property(Node) match3SuccessSecondPanel: Node;
    // ly
    initData() {
        /********************************************** */
        //加载顾客表情图片
    ...
    onLoad() {
        ...
        resources.load("json/config", JsonAsset, (err, asset: JsonAsset) => {
            ...
            // 三消成功的第一个面板
            // 绑定标题
            const match3SuccessPanelData = this.gameConfig.match3SuccessPanel;
            const match3SuccessFirstPanelTitle = this.match3SuccessFirstPanel.getChildByName("title");
            match3SuccessFirstPanelTitle.getComponent(Label).string = match3SuccessPanelData[0].title;
            // 绑定文案
            const match3SuccessFirstPanelDescription = this.match3SuccessFirstPanel.getChildByName("description");
            match3SuccessFirstPanelDescription.getComponent(Label).string = match3SuccessPanelData[0].description;
            // 绑定按钮
            const firstPanelButtonData = this.gameConfig.match3SuccessPanel[0].firstButton;
            const buttons = [
                this.match3SuccessFirstPanel.getChildByName("Button1"),
                this.match3SuccessFirstPanel.getChildByName("Button2"),
                this.match3SuccessFirstPanel.getChildByName("Button3"),
                this.match3SuccessFirstPanel.getChildByName("Button4")
            ];
            buttons.forEach((button, index) => {
                const labelNode = button.getChildByName("Label");
                const labelComponent = labelNode.getComponent(Label);
                labelComponent.string = firstPanelButtonData[index].label;
                button.on(Button.EventType.CLICK, () => {
                    this.onMatch3SuccessFirstPanelButtonClick(index + 1);
                }, this);
            });
            // 三消成功的第二面板
            this.match3SuccessFirstPanel.getChildByName("Button1").getChildByName("Label").string = "确认";
            const button1 = this.match3SuccessSecondPanel.getChildByName("Button1");
            button1.getChildByName("Label").string = "确认";
            const buttonComponent = button1.getComponent(Button);
            buttonComponent.node.on(Button.EventType.CLICK, this.onMatch3SuccessSecondPanelButton1Click, this);
                ...
                tween(children[itemindex - 2])
                    ...
                    .call(() => {
                        ...
                        if (index == 25) {
                            ...
                            // ly
                            this.match3SuccessFirstPanel.active = true;
                            // ly
                        }
    ...
    // ly
    onMatch3SuccessFirstPanelButtonClick(buttonIndex: number) {
        const secondPanelLabelData = this.gameConfig.match3SuccessPanel[0].secondButton[buttonIndex-1].label;
        const hintNode = this.match3SuccessSecondPanel.getChildByName("hint");
        if (hintNode) {
            const hintLabel = hintNode.getComponent(Label);
            if (hintLabel) {
                hintLabel.string = secondPanelLabelData;
            }
        }
        this.match3SuccessSecondPanel.active = true;
    }
    onMatch3SuccessSecondPanelButton1Click() {
        this.time += 10;
        let hour = Math.floor(this.time / 60);
        let min = this.time % 60;
        if (min < 10) {
            ...
            this.timer.getComponent(Label).string = hour + ':' + min;
        }
        this.match3SuccessSecondPanel.active = false;
    }
    // ly
}
