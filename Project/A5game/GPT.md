...
```
我现在才接手一个cocos项目，有一些bug，但我自己看代码的话，效率太低，而且由于我是才接手的别人的项目，很容易浪费时间，所以我在想，我把文件和问题给你，我们一起交流，你能不能帮我解决bug
```

```
我先问一下，.scene文件和.meta文件是什么关系
```
...
├── scenes
    ...
    ├── honor_scene.scene
    ├── main_scene.scene
    ├── game_scene.scene.meta
    ├── honor_scene.scene.meta
    ├── main_scene.scene.meta
├── scripts
    ├── AudioController.ts
    ├── Customer.ts
    ├── Food.ts
    ...
    ├── Startbtn.ts.meta
    ├── Storage.ts.meta
    └── VibrationManager.ts.meta

我想让你帮我解决bug，除了必要的问题叙述以及提供.ts文件，还有必要提供.scene文件和.meta文件吗

你觉得我需要提供哪些文件呢
```

```
好的，我会给你提供相关的ts文件，请你先挨个分析，先记住这些文件，然后我再给你说明我的问题，准备好了吗？
```

```
以上就是所有的ts文件，你先挨个审查并理解这个项目的逻辑
```

```
你不需要重新生成所有的ts代码，只需要告诉我，你在什么文件的什么地方修改了代码，然后改的代码是什么，而且为了方便理解，请你在修改的地方加上中文注释"//修改bug: ..."
```



```ts
// AudioController.ts代码如下：
...
上述代码是本次项目的全部代码，请你针对我提到的"切换顾客时存在的问题"仔细分析，并告诉我bug应该如何解决
```
